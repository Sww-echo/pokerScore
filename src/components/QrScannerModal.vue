...
async function startScanner() {
  ...
  try {
    ...
    status.value = 'error'
    localErrorText.value = error instanceof Error ? error.message : '相机权限获取失败'
    stopScanner()
  }
}
...
<template>
  <Transition name="fade">
    ...
    >
      ...
      <div
        class="relative w-full max-w-sm rounded-[2rem] border border-slate-100 bg-white p-6 shadow-2xl"
      >
        <button
          @click="closeModal"
          class="absolute right-4 top-4 flex size-8 items-center justify-center rounded-full bg-slate-100 transition-colors hover:bg-slate-200"
        >
        ...
        <div class="mb-5 mt-1 flex flex-col items-center text-center">
          <div
            ...
            <div class="relative aspect-square w-full">
              ...
              >
                <span
                  ...
                  "
                >
                  {{ status === 'loading' ? 'camera' : 'qr_code_2' }}
                </span>
                <p
              ...
              >
                <div class="relative h-48 w-48 rounded-[1.75rem] border-2 border-white/90">
                  <div class="absolute -left-0.5 -top-0.5 h-8 w-8 rounded-tl-[1.25rem] border-l-4 border-t-4 border-primary"></div>
                  <div class="absolute -right-0.5 -top-0.5 h-8 w-8 rounded-tr-[1.25rem] border-r-4 border-t-4 border-primary"></div>
                  <div class="absolute -bottom-0.5 -left-0.5 h-8 w-8 rounded-bl-[1.25rem] border-b-4 border-l-4 border-primary"></div>
                  <div class="absolute -bottom-0.5 -right-0.5 h-8 w-8 rounded-br-[1.25rem] border-b-4 border-r-4 border-primary"></div>
                  <div class="absolute left-4 right-4 top-1/2 h-0.5 -translate-y-1/2 bg-primary/80 shadow-[0_0_18px_rgba(249,212,6,0.6)]"></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
            ...
            <div class="relative">
              ...
              <input
                ...
                class="w-full rounded-xl border border-slate-200 bg-white py-4 pl-12 pr-4 text-sm font-bold text-slate-900 outline-none transition-colors focus:border-primary"
                placeholder="粘贴邀请链接或输入房间码"
                type="text"
                autocomplete="off"
                @input="emit('clearError')"
                @keyup.enter="submitManualPayload"
              />
            </div>
            <button
              @click="submitManualPayload"
              class="mt-3 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary text-sm font-black text-slate-900 transition-transform active:scale-[0.98]"
            >
